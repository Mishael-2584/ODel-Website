...
This read-only local plugin exposes the courses a lecturer can manage. ODeL calls it server-to-server; the desktop application never receives the Moodle web-service token.
...
The service starts disabled intentionally. Version `0.1.2` performs no writes to Moodle and stores no Moodle data.
