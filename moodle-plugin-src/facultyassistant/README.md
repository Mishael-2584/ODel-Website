...
10. Test `local_facultyassistant_get_user_by_email`, `local_facultyassistant_get_question_categories`, `local_facultyassistant_create_question_category`, and `local_facultyassistant_import_questions` before granting any desktop entitlement the connected features.
11. Install UEAB Course Builder `1.8.5` or later, then grant `local/facultyassistant:publishcoursebuilder` only to the dedicated Faculty Assistant service role. Confirm the external service contains `local_facultyassistant_get_course_builder` and `local_facultyassistant_publish_course_builder`, and enable the service's **Can upload files** option before enabling `coursebuilder:write` licences.
...
The service starts disabled intentionally. Version `0.7.0` adds exact email lookup for administrator-approved licence grants and Moodle XML publishing so marks, rich question metadata, and private essay grader information can be preserved without exposing Moodle service tokens.
