...
11. Install UEAB Course Builder `1.6.0` or later, then grant `local/facultyassistant:publishcoursebuilder` only to the dedicated Faculty Assistant service role. Confirm the external service contains `local_facultyassistant_get_course_builder` and `local_facultyassistant_publish_course_builder` before enabling `coursebuilder:write` licences.
