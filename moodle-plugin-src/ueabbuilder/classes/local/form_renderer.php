...
final class form_renderer {
    public static function render(array $state): string {
        ...
        $course = self::details('3. Course details',
            ...
            . self::grid(self::input('units', 'Number of units / topics', 'number', '1', '30'),
                self::input('topics', 'Topics to publish', 'number', '1', '30'))
...
function renderTopics(){captureTopics();const count=Math.max(1,Math.min(30,Number(value("topics")||9)));const wrap=byId("ubb-topics-wrap");wrap.innerHTML=Array.from({length:count},(_,i)=>topicHtml(i+1)).join("");for(let n=1;n<=count;n++){const row=topicState[n]||SAVED.topicsdata?.[n]||SAVED.topicsdata?.[String(n)]||{};TOPIC_FIELDS.forEach(key=>{const el=byId(`ubb-t${n}-${key}`);if(el&&row[key]!==undefined)el.value=row[key];});bindTopic(n);updateTopic(n);}}
