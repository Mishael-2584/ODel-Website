...
open one prompt at a time and draft an unsaved response before continuing;
facilitator guidance and supplied answers remain collapsed. These are formative
Moodle Page activities, not submitted or graded Moodle Quiz attempts.

Version 1.8.3 renders Word bold and italic emphasis as safe semantic HTML and
explicitly colours every imported heading level. This prevents Moodle theme
rules from making illustrated-study headings invisible.

Version 1.8.2 reconstructs every protected illustration URL from its persisted
Moodle course context, file item and filename when Topic Pages are rendered. A
revision is rejected if any imported image placeholder remains unresolved.
...
file area, and the block stores them in Moodle's protected file API when the
revision is published. Topic Pages render responsive, lazy-loaded figures in
their original document order. The files remain subject to course access and
are not exposed as public URLs.
...
that service; version 1.8.2 does not add another external function.
