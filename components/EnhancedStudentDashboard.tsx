  ...
  return (
    <div className="min-h-screen bg-gray-50">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="relative">
                ...
                >
                  ...
                  {studentData.statistics.messagesUnread > 0 && (
                    <span className="absolute -top-1 -right-1 h-5 w-5 bg-red-500 text-white text-xs rounded-full flex items-center justify-center">
                      {studentData.statistics.messagesUnread}
                    </span>
                  )}
                </button>
                {showNotifications && (
                  <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg border z-50">
                    ...
                    <div className="max-h-64 overflow-y-auto">
                      {studentData.messages.slice(0, 5).map((message) => (
                        <div key={message.id} className={`p-3 border-b hover:bg-gray-50 ${!message.read ? 'bg-blue-50' : ''}`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {activeTab === 'messages' && <MessagesTab data={studentData} />}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {data.enrolledCourses.map((course) => (
              <div key={course.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                ...
                <p className="text-sm text-gray-600 mb-3">{course.shortname}</p>
...
function MessagesTab({ data }: { data: StudentData }) {
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow">
        <div className="px-6 py-4 border-b border-gray-200">
          ...
          {data.messages.length > 0 ? (
            <div className="space-y-4">
              {data.messages.map((message) => (
                <div key={message.id} className={`p-4 border rounded-lg ${!message.read ? 'bg-blue-50 border-blue-200' : 'bg-gray-50'}`}>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <h4 className="font-medium text-gray-900">{message.subject}</h4>
                      <p className="text-sm text-gray-600 mt-1">{message.message}</p>
                      <p className="text-xs text-gray-500 mt-2">From: {message.from}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-gray-500">{message.date}</p>
                      {!message.read && (
